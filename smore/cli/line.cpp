...
int main(int argc, char **argv){
    ...
    if (argc == 1) {
        ...
        printf("\t-pretrain <string>\n");
        printf("\t\tThe pretrain SBERT embedding path\n");
        printf("\t-beta <float>\n");
        printf("\t\tHyperparameter of LINE_2nd_SBERT, valid only if pretrain has value; default is 0.3\n");

        printf("Usage:\n");
        printf("\n[LINE 1st]\n");
        printf("./line -train net.txt -save rep.txt -undirected 1 -order 1 -dimensions 64 -sample_times 10 -negative_samples 5 -alpha 0.025 -pretrain -beta 0.3 -threads 1\n");
        printf("\n[LINE 2nd]\n");
        printf("./line -train net.txt -save rep.txt -undirected 1 -order 2 -dimensions 64 -sample_times 10 -negative_samples 5 -alpha 0.025 -pretrain -beta 0.3 -threads 1\n");

        return 0;
    }
    
    char network_file[100], rep_file[100], pretrain_file[100]="";
    ...
    if ((i = ArgPos((char *)"-pretrain", argc, argv)) > 0) strcpy(pretrain_file, argv[i + 1]);
    ...
    if (strcmp(pretrain_file, "") != 0){
        if_sbert = true;
        line->pnet.LoadPreTrain(pretrain_file, 768, line->w_bert);
    }
    line->Train(sample_times, negative_samples, init_alpha, beta, threads, if_sbert);
