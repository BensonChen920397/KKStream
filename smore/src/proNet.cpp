...
double proNet::SbertSimilarity(vector<double>& w_bert_vertex_ptr, vector<double>& w_bert_context_ptr) {

    double sum = 0.0;
    double x_ = 0.0;
    double y_ = 0.0;

    for (int index; index < 768; index++)
    {
        sum += w_bert_vertex_ptr[index] * w_bert_context_ptr[index];
        x_ += w_bert_vertex_ptr[index] * w_bert_vertex_ptr[index];
        y_ += w_bert_context_ptr[index] * w_bert_context_ptr[index];
        // cout << w_bert_vertex_ptr[index] << "\n" <<endl;
        // cout << w_bert_context_ptr[index] << "\n" <<endl;
    }
...
void proNet::Opt_SigmoidSGD_SBERT(vector<double>& w_vertex_ptr, vector<double>& w_context_ptr, vector<double>& w_bert_vertex_ptr, vector<double>& w_bert_context_ptr, double label, int dimension, double alpha, double beta, vector<double>& loss_vertex_ptr, vector<double>& loss_context_ptr){
    // (w_vertex[vertex], w_context[context], w_bert[vertex], w_bert[context], label, dimension, alpha, back_err, w_context[context])
    int d = 0;
    double f = 0, g = 0;
    double sbert_score = 0.0;

    for (d=0; d<dimension; ++d) // prediciton
        f += w_vertex_ptr[d] * w_context_ptr[d];

    // method 1
    sbert_score = SbertSimilarity(w_bert_vertex_ptr, w_bert_context_ptr);

    // method 1
    f = f * (1.0 + beta * sbert_score);
    ...
    //method 1
    g = (label - f) * alpha * ((1.0 + beta * sbert_score));
...
void proNet::UpdatePair_Sbert(vector< vector<double> >& w_vertex, vector< vector<double> >& w_context, vector< vector<double> >& w_bert, long vertex, long context, int dimension, int negative_samples, double alpha, double beta){
    ...
    // sbert_score = SbertSimilarity(w_bert[vertex], w_bert[context]);
    // printf("%lf\n", sbert_score);
    // cout << "The sbert score of entity" << vertex << " and entity" << context << " is " << sbert_score << "\n" << endl;
    ...
    for (int neg=0; neg!=negative_samples; ++neg)
    {
        ...
        Opt_SigmoidSGD_SBERT(w_vertex[vertex], w_context[context], w_bert[vertex], w_bert[context], label, dimension, alpha, beta, back_err, w_context[context]);
        //Opt_PUSigmoidSGD(w_vertex[vertex], w_context[context], -1.0, dimension, alpha, back_err, w_context[context]);
        //Opt_PUSGD(w_vertex[vertex], w_context[context], label, alpha, 0.0, back_err, w_context[context]);
    }
    for (int d=0; d<dimension; ++d)
