...
class FastformerEncoder(nn.Module):
    def __init__(self, config=BertConfig.from_json_file('model/fastformer.json'), pooler_count=1):
...
if __name__ == '__main__':
        word_embedding = nn.Embedding(10, 768, padding_idx=0)
        embeds = word_embedding(torch.LongTensor([[1, 2, 3]]))
        print(embeds.shape)
        print(torch.tensor([[1., 1., 1.]]).shape)
        vec = torch.randn((20, 8, 128))
        ffe = FastformerEncoder()
        embeds = ffe(embeds, torch.tensor([[1., 1., 1.]]))
        print(embeds.shape)
